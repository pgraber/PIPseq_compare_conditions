...
# Make sure Phase is an ordered factor
integrated_obj$Phase <- factor(integrated_obj$Phase,
                               levels = c("G1", "G2M", "S"))
...
phase_counts <- prop.table(table(integrated_obj$Phase, integrated_obj$condition), 2) %>%
  ...
  mutate(
    Phase     = factor(Phase, levels = c("G1", "G2M", "S")),  # enforce order again
...
phase_barplot <- ggplot(phase_counts, aes(x = Condition, y = Proportion, fill = Phase)) +
  ...
  scale_fill_manual(
    values = c(G1="#999999", G2M="#E69F00", S="#56B4E9"),
    breaks = c("G1", "G2M", "S")
  ) +
  ...
  theme_pubr() +
  theme(
    plot.title      = element_text(face = "bold", size = 16),  # ↑ title size
    axis.title.y    = element_text(size = 14, face = "bold"),   # ↑ Y axis title
    axis.text.x     = element_text(size = 13, angle = 0),       # ↑ X tick labels
    axis.text.y     = element_text(size = 13),                  # ↑ Y tick labels
...
run_cp_gsea_on_de <- function(de_df, contrast,
  ...
  # order: positive NES (up) at top, most negative (down) at bottom
    # Order by NES and create sign variable
  ord <- res_top %>%
    arrange(desc(NES)) %>%
    mutate(
  ...
  max_abs_NES <- max(abs(ord$NES), na.rm = TRUE)
  x_lim <- c(-max_abs_NES, max_abs_NES)

  # old plot version (kept for reference)
  ...
  #   labs(subtitle = "Upregulated (high NES) at top; downregulated (low NES) at bottom. Color = FDR.")

  # new sign-based colour version
  p_bar <- ggplot(ord, aes(x = NES, y = Description)) +
    geom_col(aes(fill = NES_sign), width = 0.7, color = "black", linewidth = 0.1) +
    geom_vline(xintercept = 0, linetype = "dashed", linewidth = 0.4, color = "grey40") +
    ...
    labs(
      ...
      subtitle = "Positive NES = enriched in first condition; Negative NES = enriched in second.",
...
top_by_contrast <- lapply(files, function(f) {
  ...
  df %>%
    mutate(contrast = sub("^GSEA_cp_|_Hallmark\\.csv$", "", basename(f))) %>%

    filter(p.adjust <= 0.05) %>%

    arrange(desc(abs(NES))) %>%

    dplyr::select(contrast, ID, NES, p.adjust) %>%
...
# =========================
# 1) Leading-edge gene vectors (edit here if needed)
# =========================
le_control_vs_ns <- c(
  "CCN1","ID2","NTM","SPP1","JUN","RHOB","SLIT2","IGFBP2",
  "BASP1","TNC","CD59","DST","HTRA1","GJA1","GPC1","MATN2",
  "PRRX1","PFN2"
)

le_cues_vs_ns <- c(
  "CCN1","ID2","JUN","SLIT2","IGFBP2","RHOB","GJA1","NTM",
...
le_cues_vs_control <- c(
  "COLGALT1","COL4A2","EMP3","TPM1","MMP14","LAMC1","LGALS1",
...
# =========================
# 3) Categories (Core / Embedding-shared / Cue-specific)
#    Current definition:
#    - Core: present in all three contrasts
#    - Embedding-shared: present in BN and/or CN, but NOT in CB
#    - Cue-specific: any presence in CB (with or without BN/CN)
#    If you want Cue-specific == ONLY CB, see the commented variant below.
# =========================
bn <- mat[, 1] == 1
cn <- mat[, 2] == 1
cb <- mat[, 3] == 1
...
# ---- Variant (ONLY CB counts as Cue-specific) ----
# row_category <- ifelse(bn & cn & cb, "Core",
#                   ifelse((bn | cn) & !cb, "Embedding-shared",
#                   ifelse(cb & !(bn | cn), "Cue-specific", "Other")))

row_split <- factor(row_category, levels = c("Core","Embedding-shared","Cue-specific"))

# Order rows by group, then by number of contrasts present, then alphabetically
...
# Auto figure heights
n <- nrow(mat)
pdf_h <- max(6, min(18, 6 + 0.016 * n))     # gentle scaling with row count
png_h <- max(1000, min(6000, round(600 + 14 * n)))  # pixels
...
# =========================
# 6) Save helper (draw + decorate per device)
# =========================
save_ht <- function(filename, type = c("pdf","png")) {
  type <- match.arg(type)
  if (type == "pdf") {
    pdf(filename, width = 7.2, height = pdf_h, useDingbats = FALSE)
  } else {
    png(filename, width = 2300, height = png_h, res = 300)
  }

  ht_drawn <- draw(ht, padding = unit(c(6, 6, 6, 6), "mm"))

  decorate_heatmap_body("Leading-edge", {
    nr <- nrow(mat)
    split_vec <- row_split
    idx <- which(diff(as.integer(split_vec)) != 0)
    block_positions <- c(0, idx, nr)
    for (k in seq_len(length(block_positions) - 1)) {
      top <- (block_positions[k]) / nr
      bottom <- (block_positions[k + 1]) / nr
      grid.rect(
        x = 0.5, y = top + (bottom - top) / 2,
        width = 1, height = (bottom - top),
        gp = gpar(fill = NA, col = "#444444", lwd = 1.5)
      )
    }
  })
...
# 7) Write outputs
# =========================
out_dir <- here("output"); if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)

# Save matrix and categories as TSVs (useful for SI / methods)
write.table(as.data.frame(mat), file = file.path(out_dir, "LeadingEdge_presence_matrix.tsv"),
            sep = "\t", quote = FALSE, col.names = NA)
write.table(
  tibble(Gene = rownames(mat), Category = as.character(row_split)),
  file = file.path(out_dir, "LeadingEdge_gene_categories.tsv"),
  sep = "\t", quote = FALSE, row.names = FALSE
)

# Save figures
save_ht(file.path(out_dir, "LeadingEdge_genes_heatmap_nice.pdf"), type = "pdf")
save_ht(file.path(out_dir, "LeadingEdge_genes_heatmap_nice.png"), type = "png")

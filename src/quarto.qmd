...
```{r}
# ---------- GSEA with clusterProfiler (directly from res_list; no flipping) ----------
suppressPackageStartupMessages({
...
# 1) MSigDB Hallmark TERM2GENE / TERM2NAME using SYMBOLs
...
TERM2NAME  <- msig_h_tbl %>% distinct(gs_name, gs_name) %>% rename(term = gs_name, name = term)

safe_filename <- function(x) gsub("[^A-Za-z0-9._-]+", "_", x)

# 2) Runner for a single contrast data.frame from res_list
#    Expect columns: gene, avg_log2FC  (from your FindMarkers)
run_cp_gsea_on_de <- function(de_df, contrast,
  ...
  # Clean & deduplicate genes; keep strongest signal
  dd <- de_df %>%
  ...
  # Build ranked vector: keep DE orientation (avg_log2FC > 0 = up in first term of "c2_vs_c1")
  geneList <- dd$avg_log2FC
  names(geneList) <- dd$gene
  geneList <- sort(geneList, decreasing = TRUE)

  # clusterProfiler::GSEA with Hallmark (SYMBOL-based)
  ...
  # Quick plots (optional): top 15 by adjusted p
  ...
  ggsave(file.path(out_dir, paste0("GSEA_cp_dotplot_", safe_filename(contrast), ".pdf")),
         p_dot, width = 7, height = 5, useDingbats = FALSE)

  # Enrichment plot for top up & down (if present)
  top_up  <- res_df %>% filter(NES > 0) %>% slice_min(p.adjust, n = 1, with_ties = FALSE)
  top_dn  <- res_df %>% filter(NES < 0) %>% slice_min(p.adjust, n = 1, with_ties = FALSE)

  plot_one <- function(row) {
    if (nrow(row) == 0) return(NULL)
    gs <- row$ID[[1]]
    p  <- enrichplot::gseaplot2(gsea_res, geneSetID = gs, title = paste0(gs, " (", contrast, ")"))
    ggsave(file.path(out_dir, paste0("GSEA_cp_enrichment_",
           safe_filename(gs), "_", safe_filename(contrast), ".pdf")),
           p, width = 7, height = 5, useDingbats = FALSE)
  }
  plot_one(top_up); plot_one(top_dn)

  invisible(res_df)
}

# 3) Run across your in-memory DE list
out_dir_cp <- here("output","gsea_cp","conditions_from_cells")
dir.create(out_dir_cp, recursive = TRUE, showWarnings = FALSE)

iwalk(res_list, function(de_df, key) {
  # Prefer explicit contrast column if present (e.g., "BP_Cues_vs_NS")
  contrast <- if ("contrast" %in% names(de_df)) unique(de_df$contrast)[1] else {
    parts <- strsplit(key, "\\|")[[1]]           # key was "c1|c2" in your loop
    paste0(parts[2], "_vs_", parts[1])           # c2_vs_c1
  }
  message("\n=== clusterProfiler GSEA (no flip): ", contrast, " ===",
          "\n    Convention: NES>0 = enriched in FIRST term of '", contrast, "' (your c2)")
  run_cp_gsea_on_de(de_df, contrast, TERM2GENE, out_dir = out_dir_cp,
                    minGSSize = 10, maxGSSize = 500)
})

message("Done. GSEA (clusterProfiler) outputs in: ", out_dir_cp)

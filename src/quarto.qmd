...
```{r}
# ---- GSEA directly from in-memory DE (same orientation as DE; no flipping) ----
suppressPackageStartupMessages({
  library(dplyr); library(purrr); library(stringr)
  library(msigdbr); library(fgsea); library(ggplot2); library(here)
})

set.seed(123)

# Hallmark sets (uppercased symbols)
msig_h <- msigdbr(species = "Homo sapiens", collection = "H") |>
  ...
  ungroup()
gsets <- setNames(msig_h$genes, msig_h$gs_name)

safe_filename <- function(x) gsub("[^A-Za-z0-9._-]+", "_", x)

run_gsea_on_contrast_df <- function(de_df, contrast, gsets,
                                    minSize = 10, maxSize = 500,
                                    out_dir = here("output","gsea","conditions_from_cells")) {
  dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

  # Expect: gene, avg_log2FC from FindMarkers
  stopifnot(all(c("gene","avg_log2FC") %in% names(de_df)))
  ...
  # Ranks EXACTLY as in DE (avg_log2FC > 0 = up in c2 from 'c2_vs_c1')
  ranks <- dd$avg_log2FC
  names(ranks) <- dd$gene
  ranks <- sort(ranks, decreasing = TRUE)

  # Filter pathways by effective size after intersect
  eff_sizes <- vapply(gsets, function(gs) length(intersect(gs, names(ranks))), integer(1))
  gsets_eff <- gsets[eff_sizes >= minSize & eff_sizes <= maxSize]
  if (!length(gsets_eff) || length(unique(ranks)) < 10) {
    readr::write_lines("# Empty: insufficient pathway sizes or ranks",
                       file.path(out_dir, paste0("GSEA_", safe_filename(contrast), "_Hallmark.csv")))
    warning("Skipping ", contrast, " (no testable pathways).")
    return(invisible(NULL))
  }

  # Multilevel GSEA (no nperm)
  fg <- fgsea(pathways = gsets_eff, stats = ranks, minSize = minSize, maxSize = maxSize)
  out_tbl <- arrange(fg, padj)

  # Save table
  out_csv <- file.path(out_dir, paste0("GSEA_", safe_filename(contrast), "_Hallmark.csv"))
  readr::write_csv(out_tbl, out_csv)

  # Quick enrichment plots for top up/down, if present
  plot_one <- function(row) {
    if (nrow(row) == 0) return(NULL)
    gs <- row$pathway[[1]]
    p  <- fgsea::plotEnrichment(gsets_eff[[gs]], ranks) + ggtitle(paste0(gs, " (", contrast, ")"))
    ggsave(file.path(out_dir, paste0("GSEA_enrichmentPlot_",
           safe_filename(gs), "_", safe_filename(contrast), ".pdf")),
           p, width = 6, height = 4, useDingbats = FALSE)
  }
  plot_one(out_tbl %>% filter(NES > 0) %>% slice_min(padj, n = 1, with_ties = FALSE))
  plot_one(out_tbl %>% filter(NES < 0) %>% slice_min(padj, n = 1, with_ties = FALSE))

  invisible(out_tbl)
}

# Run across all DE contrasts stored in res_list
out_dir_cells <- here("output","gsea","conditions_from_cells")
dir.create(out_dir_cells, recursive = TRUE, showWarnings = FALSE)

iwalk(res_list, function(de_df, key) {
  # Prefer the explicit 'contrast' column if present (e.g., "BP_Cues_vs_NS")
  contrast <- if ("contrast" %in% names(de_df)) unique(de_df$contrast)[1] else {
    parts <- strsplit(key, "\\|")[[1]]  # key is "c1|c2"
    paste0(parts[2], "_vs_", parts[1])  # c2_vs_c1
  }
  message("\n=== GSEA (no flip): ", contrast, " ===",
          "\n    Convention: NES>0 = enriched in FIRST term of '", contrast, "' (your c2)")

  run_gsea_on_contrast_df(
    de_df, contrast, gsets,
    minSize = 10, maxSize = 500,
    out_dir = out_dir_cells
  )
})

message("Done. GSEA outputs in: ", out_dir_cells)
